...
\maketitle
\begin{abstract}
在地面基站覆盖不足区域，无人机（UAV）可作为空中移动边缘服务器，为移动用户提供计算卸载服务。本文研究任务动态到达、UAV 位置随时隙变化且机载计算与能量资源有限的场景。UAV 服务平台同时作为计算服务提供方和机制执行者，接收用户提交的私有时延敏感类型报告，并决定任务接收、服务位置及计算资源分配；用户提交类型报告并接受计算服务。双方形成动态博弈：平台以最大化长期折扣社会福利为目标，用户以最大化自身长期折扣效用为目标。由于类型报告影响任务接收和资源分配，用户可能通过策略性上报降低时延或支付，虚报则会误导平台决策，降低资源配置效率和长期社会福利。为此，本文提出由 UAV 服务平台执行的面向长期外部性的动态激励机制，将平台与用户建模为智能体，构建部分可观测随机博弈（POSG）。采用多智能体离散动作 Soft Actor-Critic（SAC）进行策略学习，由 UAV 智能体学习任务接收与轨迹控制策略，由用户智能体学习类型上报策略。机制据此联合确定任务卸载、轨迹控制和支付规则，并针对每个用户构造移除该用户的反事实环境，利用反事实长期 Q 值衡量其对其他参与者的跨期外部性并计算支付，再通过随机采样减少重复交互。仿真结果表明，在所设条件下，所提方法较即时 VCG 定价和无定价基线取得更高且更稳定的 UAV 平均训练回报，并获得更低的用户平均绝对上报误差，说明该机制能够兼顾系统收益与信息报告准确性，具有近似激励相容性。
