...
\maketitle
\begin{abstract}
在地面基站覆盖不足区域，无人机（UAV）可作为空中移动边缘服务器，为多个移动用户提供计算卸载服务。本文考虑任务动态到达、UAV 位置随时隙演化且机载计算与能量资源有限的场景。在该系统中，UAV 服务平台既是计算服务提供方，也是机制的执行主体，负责接收用户提交的私有时延敏感类型报告，并据此作出任务接收、服务位置和计算资源分配决策；移动用户作为机制参与者提交类型报告并接受计算服务。UAV 与用户因此形成动态博弈，双方优化目标不同：UAV 服务平台以最大化长期折扣社会福利为目标，用户则以最大化自身长期折扣效用为目标。由于用户的类型报告会影响任务接收和资源分配，用户有动机通过策略性上报降低自身时延或支付、提高个体效用；虚报信息会误导 UAV 决策，损害系统资源配置效率和长期社会福利。为解决上述问题，本文提出由 UAV 服务平台执行的面向长期外部性的动态激励机制，将 UAV 服务平台和各移动用户均建模为智能体，构建部分可观测随机博弈（POSG）。采用多智能体离散动作 Soft Actor-Critic（SAC）训练上述智能体：UAV 智能体学习任务接收与轨迹控制策略，各移动用户智能体学习类型上报策略。在此基础上，机制联合设计任务卸载、轨迹控制与支付规则；针对每个用户构造移除该用户的反事实环境，利用反事实长期 Q 值评估该用户对其余参与者的跨期外部性，并据此计算该用户的长期外部性支付，同时通过随机采样反事实分支减少重复环境交互。仿真结果表明，在所设条件下，所提方法相较即时 VCG 定价基线和无定价基线取得更高且更稳定的 UAV 平均训练回报，同时降低用户平均绝对上报误差，抑制策略性虚报，表现出更强的如实上报倾向和近似激励相容性。
...
然而，若将上述拍卖机制按单轮交易独立执行，其定价便难以显式反映 MEC 场景中的跨时隙状态演化。在 UAV 辅助 MEC 中，当前时隙的服务位置会改变后续时隙的 UAV--用户距离、传输速率和移动能耗，并可能影响下一轮的资源配置；因此，基于单轮即时信息确定交易结果与支付，对当前决策跨时隙影响的刻画相对有限。在此背景下，Vickrey--Clarke--Groves（VCG）拍卖以参与者对其他参与者造成的外部性为支付依据，通常用于兼顾资源配置效率与真实报告激励，是社会福利导向的经典机制。针对具有连续参与过程的联邦学习场景，Wu 等提出长期自适应 VCG 拍卖机制，将客户端选择与长期社会福利优化相结合，并考虑周期性客户端迁移\cite{wu2024longterm}。从其定价设定看，社会福利外部性主要按当前时隙计算，对当前报告或参与决策可能带来的后续 UAV 位置、传输速率、任务时延和移动能耗影响未作显式刻画。对于本文的动态 UAV 辅助 MEC 场景，可进一步将这些跨时隙影响纳入外部性评估。
...
\begin{itemize}
    \item 面向任务动态到达、UAV 位置随时隙演化且机载资源受限的 UAV 辅助 MEC 场景，构建包含 UAV 服务平台和移动用户的长期动态激励模型，将用户私有时延敏感类型、任务接收与服务位置决策以及支付规则纳入统一的长期社会福利分析框架。
    \item 提出基于反事实长期 Q 值的跨期外部性定价方法。通过比较完整环境与移除特定用户后的反事实环境，刻画该用户对其他参与者长期累计福利的影响，从而将传统即时 VCG 定价扩展到具有状态演化的动态卸载过程。
    \item 设计基于多智能体离散动作 SAC 的策略学习与支付计算框架，分别学习 UAV 的任务接收和服务位置策略以及移动用户的类型上报策略，并通过随机采样反事实分支减少长期外部性估计所需的重复环境交互。
\end{itemize}
