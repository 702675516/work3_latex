...
\maketitle
\begin{abstract}
无人机（UAV）辅助移动边缘计算可为地面基站覆盖不足区域提供灵活的计算卸载服务，但其长期高效运行需要兼顾有限机载资源的合理配置与自利用户的有效激励。在任务动态到达且计算资源竞争的场景下，拥有私有类型信息的用户可能通过策略性上报谋求更高效用。具体而言，本文构建由一架 UAV 和多个移动用户组成的时隙化计算卸载系统，综合考虑空地通信、任务执行与 UAV 能耗，其中用户上报私有时延敏感类型，UAV 据此确定任务接收与服务位置，并按任务量比例分配机载计算资源。在此基础上，构建面向长期社会福利优化的动态激励机制：系统侧以最大化长期折扣社会福利为目标，用户侧以最大化自身长期折扣效用为目标，据此联合设计任务卸载、UAV 轨迹控制与支付机制。为求解上述问题，本文将连续交互建模为部分可观测随机博弈（POSG），并采用多智能体离散动作 Soft Actor-Critic（SAC）分别学习用户上报策略以及 UAV 任务接收与轨迹控制策略。同时，基于反事实长期 Q 值估计用户的跨期外部性并构造长期外部性支付，通过随机采样反事实分支减少重复环境交互。仿真结果表明，在所设条件下，所提方法相较即时 VCG 定价基线和无定价基线获得了更高的 UAV 平均训练回报；相较即时 VCG 定价基线，用户总体平均绝对上报误差由 $0.185$ 降至 $0.126$，并呈现近似激励相容性。
...
从机制设计视角看，资源配置与支付规则可在统一框架下联合设计。Xiao 等结合双边拍卖和组合拍卖，联合刻画多用户、多服务器间的双向竞价与多维资源分配\cite{xiao2023multiround}；Li 等则针对任务价值随时间衰减的动态竞争场景，提出一种通过短时隙优化、单调赢家确定和临界价格支付提升总体效用并抑制虚假交易的机制\cite{li2025incentive}。相较于侧重供需撮合的双边拍卖，Vickrey--Clarke--Groves（VCG）机制更加关注资源配置的整体效率，旨在实现社会福利最大化。在联邦学习场景中，Wu 等针对连续训练过程中的客户端参与问题，提出长期自适应 VCG 机制，将客户端选择与长期社会福利优化相结合\cite{wu2024longterm}。然而，在边缘计算卸载的连续决策过程中，当前时隙的上报可能影响后续资源分配；该机制的定价主要依据当期即时福利，对跨期外部性的显式刻画相对有限。

因此，本文研究 UAV 辅助 MEC 网络中的长期动态激励与卸载优化问题，借鉴 Bergemann 和 Välimäki 提出的动态枢轴机制\cite{bergemann2010dynamic}，构建以长期社会福利最大化为目标的动态激励机制。本文采用 SAC 算法学习 UAV 与移动用户的决策策略，并利用长期 Q 函数估计用户的动态外部性，据此构造长期外部性支付，以实现对用户长期贡献的合理定价和系统长期社会福利最大化。

本文其余部分安排如下：第二节介绍系统模型；第三节给出动态激励机制建模；第四节设计相应的学习与支付算法；第五节给出仿真结果与分析。
...
用户的真实时延敏感系数 $\theta_n$ 从集合 $\{0.2,0.5,0.8\}$ 中独立均匀采样。各时隙中，任务规模 $D_n(t)$ 从集合 $\{100,200,300\}$ KB 中独立均匀采样。每比特所需 CPU 周期数设为 $\xi=1000$，UAV 与本地设备的 CPU 频率分别设为 $F_{\mathrm{uav}}=0.1$ GHz 和 $f_{\mathrm{loc}}=1.5$ GHz。
